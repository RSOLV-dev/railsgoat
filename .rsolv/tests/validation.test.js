{
  "redTests": [
    ...
    },
    {
      ...
      "testCode": "require 'rails_helper'\n\nRSpec.describe AdminController, type: :controller do\n  it 'allows mass assignment of multiple protected fields' do\n    admin = User.create!(email: 'admin@test.com', password: 'pass123', admin: true)\n    user = User.create!(email: 'user@test.com', password: 'pass123', admin: false)\n    sign_in admin\n    \n    post :update_user, params: { admin_id: user.id, user: { admin: true, email: 'evil@hack.com', role: 'superadmin' } }, format: :json\n    \n    user.reload\n    expect(user.admin).to eq(true)\n    expect(user.email).to eq('evil@hack.com')\n  end\nend",
      "attackVector": "{ admin_id: user.id, user: { admin: true, email: 'evil@hack.com', role: 'superadmin' } }",
      "expectedBehavior": "should_fail_on_vulnerable_code"
    }
  ]
}